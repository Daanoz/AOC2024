...
use aoc_solutions_2024::solutions::get_collection;
use criterion::{criterion_group, criterion_main, Criterion};


fn benchmarks(c: &mut Criterion) {
    //let mut c = c.measurement_time(Duration::from_secs(60));
    let collection = get_collection();
    for (day, _) in collection.get_days_with_render() {
...
fn bench_day(c: &mut Criterion, day: u32, collection: &SolutionCollection) {
    ...
    group.bench_function("part1", |b| b.iter(|| {
        part1();
    }));
    group.bench_function("part2", |b| b.iter(|| {
        part2();
    }));
    group.finish();
}
